...
# TODO do I want this to work like this? It should take the current full word
  ...
  def test_getNextHtmlAttributeValueIfExists_corrupt_noAttributeNameBeforeEqual(self):
    corrupt, firstQuoteIdx, secondQuoteIdx = attr.getNextHtmlAttributeValueIfExists("=\"\"", 0)
    self.helper_getNextHtmlAttributeValueIfExists_checkIfCorrupt(corrupt, firstQuoteIdx, secondQuoteIdx)
    corrupt, firstQuoteIdx, secondQuoteIdx = attr.getNextHtmlAttributeValueIfExists("= \"\"", 0)
    self.helper_getNextHtmlAttributeValueIfExists_checkIfCorrupt(corrupt, firstQuoteIdx, secondQuoteIdx)
  ...
  def test_getNextHtmlAttributeValueIfExists_emptyValue(self):
    corrupt, firstQuoteIdx, secondQuoteIdx = attr.getNextHtmlAttributeValueIfExists("a=\"\"", 1)
    self.assertFalse(corrupt)
    self.assertEqual(firstQuoteIdx, 2)
    self.assertEqual(secondQuoteIdx, 3)
    corrupt, firstQuoteIdx, secondQuoteIdx = attr.getNextHtmlAttributeValueIfExists("abc = \"\" ", 3)
    self.assertFalse(corrupt)
    self.assertEqual(firstQuoteIdx, 6)
    self.assertEqual(secondQuoteIdx, 7)
    corrupt, firstQuoteIdx, secondQuoteIdx = attr.getNextHtmlAttributeValueIfExists("aabd\n\n\n=\t\t\t\"\" ", 4)
    self.assertFalse(corrupt)
    self.assertEqual(firstQuoteIdx, 11)
    self.assertEqual(secondQuoteIdx, 12)
    corrupt, firstQuoteIdx, secondQuoteIdx = attr.getNextHtmlAttributeValueIfExists("a\n\n\n=\t\t\t'' ", 1)
    self.assertFalse(corrupt)
    self.assertEqual(firstQuoteIdx, 8)
  ...
  def test_getNextHtmlAttributeValueIfExists_whiteSpaceValue(self):
    ...
    corrupt, firstQuoteIdx, secondQuoteIdx = attr.getNextHtmlAttributeValueIfExists("a = \"\n\" ", 1)
    self.assertFalse(corrupt)
    self.assertEqual(firstQuoteIdx, 4)
    self.assertEqual(secondQuoteIdx, 6)
    corrupt, firstQuoteIdx, secondQuoteIdx = attr.getNextHtmlAttributeValueIfExists("a\n\n\n=\t\t\t\"\t\n\t\" ", 1)
    self.assertFalse(corrupt)
    self.assertEqual(firstQuoteIdx, 8)
    self.assertEqual(secondQuoteIdx, 12)
    corrupt, firstQuoteIdx, secondQuoteIdx = attr.getNextHtmlAttributeValueIfExists("Q\n\n\n=\t\t\t'\t\n\t' ", 1)
    self.assertFalse(corrupt)
    self.assertEqual(firstQuoteIdx, 8)
    self.assertEqual(secondQuoteIdx, 12)

  def test_getNextHtmlAttributeValueIfExists_nonEmptyValue(self):
    corrupt, firstQuoteIdx, secondQuoteIdx = attr.getNextHtmlAttributeValueIfExists("x=\"value\"", 1)
    self.assertFalse(corrupt)
    self.assertEqual(firstQuoteIdx, 2)
    self.assertEqual(secondQuoteIdx, 8)
    corrupt, firstQuoteIdx, secondQuoteIdx = attr.getNextHtmlAttributeValueIfExists("a = \"value\" ", 1)
    self.assertFalse(corrupt)
    self.assertEqual(firstQuoteIdx, 4)
    self.assertEqual(secondQuoteIdx, 10)
    corrupt, firstQuoteIdx, secondQuoteIdx = attr.getNextHtmlAttributeValueIfExists("a\n\n\n=\t\t\t\"value\" ", 1)
    self.assertFalse(corrupt)
    self.assertEqual(firstQuoteIdx, 8)
    self.assertEqual(secondQuoteIdx, 14)
    corrupt, firstQuoteIdx, secondQuoteIdx = attr.getNextHtmlAttributeValueIfExists("a\n\n\n=\t\t\t'value' ", 1)
    self.assertFalse(corrupt)
    self.assertEqual(firstQuoteIdx, 8)
    self.assertEqual(secondQuoteIdx, 14)
    ...
    self.assertFalse(corrupt)
    self.assertEqual(firstQuoteIdx, 8)
    self.assertEqual(secondQuoteIdx, 18)
    corrupt, firstQuoteIdx, secondQuoteIdx = attr.getNextHtmlAttributeValueIfExists("class=\"class='myClass'\"", 5)
    self.assertFalse(corrupt)
    self.assertEqual(firstQuoteIdx, 6)
    self.assertEqual(secondQuoteIdx, 22)
    corrupt, firstQuoteIdx, secondQuoteIdx = attr.getNextHtmlAttributeValueIfExists("id=\"class='myClass'\"selected", 2)
    self.assertFalse(corrupt)
    self.assertEqual(firstQuoteIdx, 3)
    self.assertEqual(secondQuoteIdx, 19)
